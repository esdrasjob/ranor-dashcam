...
async def main(args):
    ...
    public_jt1078_port = args.public_jt1078_port or args.jt1078_port
    jt1078_srv  = JT1078StreamServer(
        event_bus, storage,
        host='0.0.0.0', port=args.jt1078_port,
        public_host=public_host,
        public_port=public_jt1078_port,
    )
    ...
    pub = f"{public_host or 'auto'}:{public_jt1078_port}"
    logger.info(f"✅ JT1078 → tcp://0.0.0.0:{args.jt1078_port}  (stream de vídeo | público: {pub})")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--public-host', type=str, default=None,
                        help='IP/host público para stream (ex: 0.tcp.sa.ngrok.io). '
                             'Se não informado, detecta IP local automaticamente.')
    parser.add_argument('--public-jt1078-port', type=int, default=None,
                        help='Porta pública do JT1078 (ex: porta ngrok). '
                             'Se não informado, usa --jt1078-port.')
