...
function addEvent(event) {
  ...
  } else if (t === 'transparent_data') {
    detail = `📦 TIPO ${d.type} | HEX: ${d.data_hex}`;
    badge = `<span class="badge" style="background:#2d3348;color:#00d4ff">DADOS</span>`;
  } else {
...
def create_api(storage: Path, event_bus, jt808_server, jt1078_server=None) -> FastAPI:
    ...
    @app.get("/api/debug/packets/{phone}")
    async def debug_packets(phone: str):
        """Mostra detalhes dos últimos pacotes recebidos do device"""
        session = jt808_server.sessions.get(phone)
        if not session:
            raise HTTPException(404, f"Dispositivo {phone} não conectado")
        ...
        return {
            ...
            "last_location": {
                "alarm_flags_hex": f"0x{loc.alarm_flags:08X}" if loc else None,
                "alarm_flags_bin": f"{loc.alarm_flags:032b}" if loc else None,
                "alarm_flags_dec": loc.alarm_flags if loc else None,
                "active_alarms": loc.alarms if loc else [],
                "status_flags_hex": f"0x{loc.status_flags:08X}" if loc else None,
                "active_status": loc.status if loc else [],
    ...
    @app.post("/api/debug/inject-alarm/{phone}")
    async def inject_test_alarm(phone: str, alarm_type: str = "camera_occlusion"):
        """Injeta um alarme de teste no event bus (sem precisar da câmera)"""
        ALARM_TYPES = {
            "camera_occlusion": {"flags": 1 << 18, "label": "Câmera obstruída"},
            "collision":        {"flags": 1 << 11, "label": "Colisão (acelerômetro)"},
            "driver_abnormal":  {"flags": 1 << 19, "label": "Motorista anormal (IA)"},
            "overspeed":        {"flags": 1 << 1,  "label": "Excesso de velocidade"},
            "emergency":        {"flags": 1 << 0,  "label": "Emergência"},
        }
        info = ALARM_TYPES.get(alarm_type, ALARM_TYPES["camera_occlusion"])
        session = jt808_server.sessions.get(phone)
        loc = session.last_location if session else None
        payload = {
            "phone": phone,
            "lat": loc.latitude if loc else -20.7189,
            "lon": loc.longitude if loc else -46.6097,
            "speed": loc.speed if loc else 0,
            "direction": 0,
            "altitude": 0,
            "timestamp": datetime.now().isoformat(),
            "alarms": [info["label"]],
            "status": ["ACC ligado", "Posicionado"],
            "extras": {},
            "alarm_list": [info["label"]],
            "injected": True,
        }
        await event_bus.publish("alarm", payload)
        return {"status": "ok", "alarm": info["label"], "flags": f"0x{info['flags']:08X}"}
